...
void MyTcpSocket::recvMsg()  //接收数据
{
    if(!m_bUpload)   //不是上传文件用pdu接收数据
    {
        ...
        case ENUM_MSG_TYPE_UPLOAD_FILE_REQUEST:
        {
            ...
            qint64 fileSize = 0;
            sscanf(pdu->caData, "%s %lld", caFileName, &fileSize);   //得到文件名和大小
            char *pPath = new char[pdu->uiMSgLen];
            memcpy(pPath, pdu->caMsg, pdu->uiMSgLen);
    ...
    else
    {
        ...
        //接收上传文件的数据
        QByteArray buff = readAll();  //接收读到数据
        m_file.write(buff);
        m_iRecved += buff.size();
        ...
        else if(m_iTotal > m_iRecved)
        {
            m_file.close();
            m_bUpload = false;
            strcpy(respdu->caData, UPLOAT_FILE_FAILURED);  //失败
            write((char*)respdu, respdu->uiPDULen);
            free(respdu);
            respdu = NULL;
        }
